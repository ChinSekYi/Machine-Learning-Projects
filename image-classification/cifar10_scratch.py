...
transform = transforms.Compose([ 
    ...
    transforms.Normalize((0.5, 0.5, 0.5),(0.5, 0.5, 0.5))] #(mean, sd) for each of the tree RGB colours. 

) 

  

batch_size = 4 #higher the batch size, the more accurate it is but takes up more memory space 

  

num_workers = 2 #if positive int -> Pytorch switch to perform multi-process data loading  

                #eg. 2 workers simultaneouly putting data into the computer's RAM. Allowd speed up of training process by utilising machines with multiple cores. 
...
for epoch in range(2): #an epoch is one pass over the entire train set. Too many epochs may lead to overfitting 

  

    running_loss = 0.0 

    for i, data in enumerate(trainloader, 0): #i -> batch number, data = [inputs, labels] 
...
x = torch.tensor(2.0, requires_grad=True); print(x) 
